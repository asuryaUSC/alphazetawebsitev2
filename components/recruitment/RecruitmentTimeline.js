...
import { Coffee, Users, FileText, Calendar, UserCheck, HelpCircle } from 'lucide-react';
...
const RecruitmentTimeline = () => {
  ...
  const timelineEvents = [
    ...
    },
    {
      icon: UserCheck,
      title: 'Interviews',
      date: 'September 15 - September 16',
  ...
  return (
    <section ref={ref} className="relative bg-gradient-to-br from-[#E5F2FF] via-[#EDF5FF] to-[#F0F8FF] py-12 md:py-16 px-4 md:px-6 overflow-hidden" id="az-groups">
                ...
                <div className={`w-full pl-16 md:pl-0 md:w-5/12 ${
                  ...
                  <div className="bg-white/80 backdrop-blur-sm rounded-xl p-5 md:p-6 shadow-lg border border-[#89CFF0]/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1 hover:bg-white/90">
                    <div className={`flex items-center mb-3 ${
                      ...
                      <h3 className={`text-lg md:text-xl font-bold text-[#3D2930] ${
                        index % 2 === 0 ? 'order-first md:order-last' : 'order-first'
                      }`}>{event.title}</h3>
                    </div>
                    <div className={`text-base md:text-lg font-semibold text-[#89CFF0] mb-2 ${
                      index % 2 === 0 ? 'text-left md:text-right' : 'text-left'
                    }`}>{event.date}</div>
                    <p className={`text-[#3D2930]/80 text-sm md:text-base leading-relaxed ${
                      index % 2 === 0 ? 'text-left md:text-right' : 'text-left'
                    }`}>{event.description}</p>
                  </div>
                </div>
